...
# Explicit ticket references ("ticket 75919", "bug #75919", "#75919") and bare
# 5-digit ticket-range numbers — pinned into context by ticket_id.
_TICKET_ID_RE = re.compile(r"(?:ticket|tickets|bug|#)\s*#?\s*(\d{3,7})", re.IGNORECASE)
_BARE_ID_RE = re.compile(r"(?<![\d.])\b(\d{5})\b(?![\d.])")
...
def _extract_ticket_ids(text: str) -> list[str]:
    out: list[str] = []
    for rx in (_TICKET_ID_RE, _BARE_ID_RE):
...
def handle_turn(user_msg: str, session: Session, filters: Filters,
    ...
    carried: list[Chunk] = []
    if _is_reference_followup(user_msg) and session.last_context_ids:
        carried = deps.retriever.get_by_ids(session.last_context_ids)
    augment = _merge_chunks(pinned, carried, limit=config.TOP_K_RERANK)
    skip_clarify = False
    if augment:
        confident_fresh = (result.chunks if (not result.abstain_reason
                           and result.rerank_top_score >= LOW_CONFIDENCE_CEILING) else [])
        result.chunks = _merge_chunks(augment, confident_fresh, limit=config.TOP_K_RERANK)
