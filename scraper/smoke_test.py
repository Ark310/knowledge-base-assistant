...
def main() -> None:
    ...
    n_imgs = sum(len(c.get("images") or []) for c in comments)
    n_imgs_saved = sum(1 for c in comments for im in (c.get("images") or []) if im.get("saved_path"))
    print(f"  {chk(n_imgs == 0 or n_imgs_saved > 0)}comment images: {n_imgs_saved}/{n_imgs} saved to disk")
